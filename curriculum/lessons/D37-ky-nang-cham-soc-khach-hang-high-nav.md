# LỘ TRÌNH ĐÀO TẠO & THỰC CHIẾN NEXTGEN 2026
## NGÀY 37 (D37): KỸ NĂNG CHĂM SÓC KHÁCH HÀNG HIGH-NAV & BÁO CÁO TỐT NGHIỆP
### MÃ TÀI LIỆU: DT-37
*Chương trình hợp tác đào tạo giữa FinPeace và KB Securities Vietnam – Sở Giao Dịch 3 (HO3)*
> 📥 **Tài liệu thực hành:** [Bảng Tra Cứu Nhanh (Cheatsheet PDF)](file:///Users/yenle/Documents/kbsvnextgen/curriculum/lessons/cheatsheets/D37-cheatsheet-ky-nang-cham-soc-khach-hang-high-nav.html) | [Bản in sạch (Skill Resource)](file:///Users/yenle/Documents/kbsvnextgen/.agents/skills/kbsv-nextgen/resources/03-dao-tao/lessons/cheatsheets/D37-cheatsheet-ky-nang-cham-soc-khach-hang-high-nav.html)


---

## 📌 I. THÔNG TIN BÀI HỌC & MỤC TIÊU (LESSON OVERVIEW & OBJECTIVES)

*   **Thời lượng học:** Cả ngày (bám sát Daily Routine: Morning Briefing → Content Lab → High-NAV CRM Audit → Graduation Report Drafting).
*   **Vị trí trong lộ trình:** Ngày thứ ba mươi bảy của Lộ trình (Tuần 6 - Chăm sóc khách hàng lớn và Tốt nghiệp).
*   **Mục tiêu bài học theo khung ASK:**
    *   **Attitude (Thái độ):** Tư duy cẩn trọng, bảo mật và cực kỳ tinh tế. Thể hiện sự tôn trọng tuyệt đối đối với sự riêng tư và bảo mật thông tin của khách hàng lớn. Luôn khiêm tốn, lắng nghe sâu sắc, tuyệt đối không vồ vập, săn đón thô thiển. Ý thức rõ tầm quan trọng của việc hoàn thành Báo cáo tốt nghiệp thực chiến.
    *   **Skills (Kỹ năng):** Thiết kế danh mục phân bổ tài sản cá nhân hóa (Tailor-made Portfolio) chuẩn Private Banking Style cho khách hàng lớn; viết báo cáo phân tích hiệu suất tài sản định kỳ; soạn thảo hoàn chỉnh Đề cương báo cáo tốt nghiệp NextGen HO3.
    *   **Knowledge (Kiến thức):** Nắm rõ tâm lý học hành vi và nỗi đau tài chính đặc thù của nhóm khách hàng High-NAV (NAV ≥ 1 tỷ đồng). Hiểu sâu cấu trúc và yêu cầu bảo vệ của Báo cáo tốt nghiệp thực chiến.

---

## 📌 II. LÝ THUYẾT CHUYÊN SÂU (THEORETICAL FOUNDATION)

### 1. Tâm Lý Học Và Nhu Cầu Đặc Thù Của Khách Hàng High-NAV

Tại Sở Giao Dịch 3 (HO3), khách hàng High-NAV được định nghĩa là các cá nhân có tổng quy mô tài sản ròng (NAV) giao dịch thực tế tại tài khoản lớn hơn **1 tỷ đồng**.

Đối với phân khúc khách hàng lớn này, Broker NextGen tuyệt đối không được áp dụng tư duy bán hàng kiểu phím hàng ăn xổi lướt sóng ngắn hạn T+. Giới siêu giàu và người có tài sản lớn họ cực kỳ nhạy cảm và có tâm lý học hành vi đặc thù:
*   **Ưu tiên Bảo tồn vốn:** Nỗi sợ lớn nhất của họ là mất tiền. Họ đã trải qua nhiều thăng trầm để tích lũy được khối tài sản này, do đó họ cần sự an toàn trước lạm phát và suy thoái kinh tế hơn là việc nhân đôi tài khoản mạo hiểm.
*   **Cần sự thấu cảm và lắng nghe sâu:** Họ không cần một người suốt ngày gọi điện nhắc đặt lệnh mua bán. Họ cần một **Wealth Advisor (Cố vấn tài sản)** biết lắng nghe các mục tiêu cuộc đời của họ (lo cho con đi du học, chuẩn bị quỹ hưu trí an nhàn, chuyển giao tài sản thừa kế) để thiết kế giải pháp tài chính tổng thể.
*   **Đòi hỏi tính bảo mật cực cao:** Mọi thông tin về số dư tài sản, nợ vay Margin hay thông tin cá nhân của họ phải được bảo vệ tuyệt đối như một bí mật quốc gia.

---

### 2. Quy Trình Chăm Sóc Cá Nhân Hóa Chuẩn Private Banking Style

Broker NextGen áp dụng quy trình 3 bước bọc thép để chinh phục và giữ chân khách hàng High-NAV:

```
               QUY TRÌNH CHĂM SÓC KHÁCH HÀNG HIGH-NAV:
┌─────────────────────────┐     ┌─────────────────────────┐     ┌─────────────────────────┐
│ BƯỚC 1: LẮNG NGHE SÂU   │ ──> │ BƯỚC 2: THIẾT KẾ RIÊNG  │ ──> │ BƯỚC 3: BÁO CÁO ĐỊNH KỲ │
├─────────────────────────┤     ├─────────────────────────┤     ├─────────────────────────┤
│ Gặp gỡ, lắng nghe mục   │     │ Thiết kế danh mục phân  │     │ Gửi báo cáo hiệu suất   │
│ tiêu cuộc đời, lập hồ   │     │ bổ tài sản Tailor-made  │     │ tài sản hàng tháng bằng │
│ sơ khẩu vị rủi ro.      │     │ kết hợp tiết kiệm, CP.  │     │ văn bản chỉn chu.       │
└─────────────────────────┘     └─────────────────────────┘     └─────────────────────────┘
```

*   **Bước 1: Lắng nghe sâu & Lập hồ sơ (KYC nâng cao):** Gặp gỡ trực tiếp (hoặc phòng Zoom riêng). Lắng nghe cấu trúc tài sản hiện có của họ (bất động sản, tiết kiệm, vàng) và ghi nhận khẩu vị rủi ro chi tiết qua khung câu hỏi định lượng.
*   **Bước 2: Thiết kế danh mục phân bổ tài sản riêng biệt (Tailor-made Portfolio):** Thiết lập bản kế hoạch phân bổ tài sản cá nhân hóa dựa trên Tháp tài sản 3 tầng của FinPeace. Phân chia rõ ràng phần tiền gửi an toàn, phần trái phiếu tạo dòng tiền đều, phần cổ phiếu tích sản bọc thép (ACB, FPT) và phần chứng chỉ quỹ Diamond ETF.
*   **Bước 3: Báo cáo hiệu suất định kỳ:** Định kỳ ngày 5 hàng tháng, Broker gửi một bản báo cáo PDF thiết kế chỉn chu chuẩn màu thương hiệu HO3 tóm tắt biến động NAV, số tiền cổ tức nhận về và nhận định vĩ mô ngắn gọn.

---

### 3. Cấu Trúc Đề Cương Báo Cáo Tốt Nghiệp NextGen HO3

Hoàn thành **Báo cáo tốt nghiệp thực chiến** là điều kiện bắt buộc cuối cùng để học viên được cấp chứng chỉ tốt nghiệp dự án NextGen 2026 và chính thức ký hợp đồng bổ nhiệm làm Broker chính thức tại Sở Giao Dịch 3 (HO3).

Bản báo cáo tốt nghiệp bắt buộc phải viết chi tiết tối thiểu 3.000 từ bám sát cấu trúc sau:
1.  **Phần 1: Nhật ký thực chiến 42 ngày:** Tổng hợp quá trình thực thi Daily Routine (Morning Briefing → Content Lab → Livestream 90 phút). Các bài học kinh nghiệm xử lý khủng hoảng khách hàng.
2.  **Phần 2: Báo cáo hiệu quả phễu chuyển đổi:** Bảng số liệu định lượng chi tiết:
    *   Tổng số lượng tài khoản eKYC mở mới thu được qua các kênh.
    *   Tổng số lượng tài khoản active Standard (NAV ≥ 5 triệu).
    *   Quy mô NAV huy động lũy kế và tổng Doanh thu thuần (DTT) mang về cho HO3.
3.  **Phần 3: Bài toán hoạch định tài sản thực tế (Case Study):** Trình bày chi tiết cách Broker thiết lập Tháp tài sản và danh mục phân bổ cho một khách hàng thực tế đã chốt thành công trong quá trình thực tập tại HO3.

---

### 4. Câu Chuyện Thực Tế: Cách Broker Minh Chinh Phục Tài Khoản 5 Tỷ Đồng Của Cô Lan Nhờ Báo Cáo Định Kỳ

Minh (23 tuổi, Broker NextGen HO3) tiếp cận được cô Lan (58 tuổi, chủ doanh nghiệp may mặc đã về hưu, nhóm tính cách **S**) qua một buổi Seminar offline tại văn phòng. Cô Lan đang có 5 tỷ đồng tiền gửi tiết kiệm chuẩn bị đáo hạn và đang bị 3 nhân viên môi giới của các công ty chứng khoán khác gọi điện săn đón nài nỉ cô mở tài khoản lướt sóng phái sinh cam kết lãi khủng. Cô Lan rất sợ hãi và có ác cảm lớn với chứng khoán.

Minh đã áp dụng quy trình chăm sóc tinh tế chuẩn HO3 để thuyết phục cô Lan:
1.  **Lắng nghe thấu cảm:** Minh hẹn gặp cô Lan tại một quán trà yên tĩnh. Suốt buổi gặp, Minh không hề nhắc đến một mã cổ phiếu nào. Anh chỉ lắng nghe cô chia sẻ về nỗi lo lạm phát bào mòn tiền tiết kiệm và mong muốn có dòng tiền đều đặn hàng năm để an dưỡng tuổi già không phiền lụy con cháu.
2.  **Thiết kế danh mục bọc thép:** Minh thiết kế bản kế hoạch phân bổ tài sản riêng biệt cho cô Lan:
    *   50% gửi tiết kiệm và mua trái phiếu chính phủ an toàn lấy dòng tiền đều đặn hàng tháng (Vùng bảo vệ).
    *   40% mua gom tích sản cổ phiếu ngân hàng ACB và FPT để hưởng cổ tức tiền mặt bền vững (Vùng tích sản).
    *   10% mua chứng chỉ quỹ Diamond ETF FUEVFVND để tăng trưởng (Vùng tăng trưởng).
3.  **Thực thi chuyên nghiệp:** Minh gửi cho cô Lan bản báo cáo tài sản định kỳ chỉn chu hàng tháng gửi qua email cá nhân, ghi rõ mức tăng trưởng NAV và lịch chi trả cổ tức thực tế.

```
TIẾN TRÌNH CHINH PHỤC KHÁCH HIGH-NAV CỦA MINH:
[ Cô Lan có 5 tỷ gửi tiết kiệm ] ──(Minh lắng nghe thấu cảm mục tiêu hưu trí)──> [ Thiết kế danh mục 3 tầng ]
                                                                                      │
                                                                           (Báo cáo NAV định kỳ ngày 5)
                                                                                      │
                                                                                      ▼
[ Trở thành khách VIP trung thành ] <─── (Chuyển 5 tỷ về HO3) ─── [ Cảm giác bình an vượt trội đối thủ ]
```

*Kết quả:* Sự điềm tĩnh, khoa học và tinh tế của Minh đã đánh gục hoàn toàn các đối thủ săn đón ồn ào kia. Cô Lan đã quyết định chuyển dịch toàn bộ **5 tỷ đồng** về tài khoản Standard tại KBSV HO3 giao dịch dưới sự cố vấn của Minh. Cô chia sẻ: *"Làm việc với cháu cô thấy rất bình an, không bị áp lực xanh đỏ hàng ngày mà tiền của cô vẫn sinh sôi đều đặn"*.

---

## 📌 III. BÀI TẬP THỰC HÀNH TÍNH TOÁN & BỐC TÁCH (PRACTICAL EXERCISES)

### 📝 Bài Tập: Thiết Thiết Kế Danh Mục Phân Bổ Tài Sản & Dự Phóng Dòng Tiền Hưu Trí Cho Khách Hàng High-NAV

**Hồ sơ khách hàng chú Hùng:**
*   Chú Hùng (60 tuổi, chuẩn bị nghỉ hưu trí) có quy mô vốn nhàn rỗi **3.000.000.000 VNĐ** muốn gửi gắm tại KBSV HO3.
*   **Mục tiêu đầu tư:** Bảo tồn vốn tuyệt đối, cần có nguồn dòng tiền đều đặn hàng năm khoảng **150.000.000 VNĐ** để trang trải chi phí sinh hoạt tuổi già mà không cần bán vào tài sản gốc.
*   **Khẩu vị rủi ro:** Thận trọng (Thấp).

**Yêu cầu đối với học viên:**
1.  Lập bảng thiết kế danh mục phân bổ tài sản chi tiết cho chú Hùng theo cấu trúc Tháp tài sản 3 tầng của FinPeace:
    *   **Vùng 1 (Bảo vệ - Gửi tiết kiệm & Trái phiếu an toàn):** Phân bổ tỷ lệ **50%** tổng vốn (lãi suất suất bình quân đạt **6%/năm**).
    *   **Vùng 2 (Tích sản - ACB & FPT):** Phân bổ tỷ lệ **40%** tổng vốn (giả định tỷ suất cổ tức tiền mặt đều đặn hàng năm nhận được là **6.5%/năm** trên giá trị vốn đầu tư gốc).
    *   **Vùng 3 (Tăng trưởng - Diamond ETF FUEVFVND):** Phân bổ tỷ lệ **10%** tổng vốn (tỷ suất tăng trưởng kỳ vọng đạt **12%/năm**, không chi trả cổ tức tiền mặt mà tự động tái đầu tư).
2.  Tính toán số tiền giải ngân thực tế cho từng vùng tài sản.
3.  Tính tổng dòng tiền đều đặn hàng năm (tiền mặt thực tế thu về từ lãi tiết kiệm/trái phiếu và cổ tức ACB/FPT) chú Hùng nhận được. Đánh giá xem có đạt mục tiêu dòng tiền 150 triệu của chú Hùng đề ra ban đầu không.

#### 💡 Lời Giải Chi Tiết Của Bài Tập:

1.  **Bước 1: Tính số tiền giải ngân thực tế cho từng vùng tài sản:**
    *   **Vùng 1 (Bảo vệ - 50%):**
        $$\text{Tiền Vùng 1} = 3.000.000.000 \times 50% = 1.500.000.000\text{ VNĐ}$$
    *   **Vùng 2 (Tích sản - 40%):**
        $$\text{Tiền Vùng 2} = 3.000.000.000 \times 40% = 1.200.000.000\text{ VNĐ}$$
    *   **Vùng 3 (Tăng trưởng - 10%):**
        $$\text{Tiền Vùng 3} = 3.000.000.000 \times 10% = 300.000.000\text{ VNĐ}$$

2.  **Bước 2: Tính dòng tiền mặt thu về hàng năm của chú Hùng:**
    *   Dòng tiền từ Vùng 1 (Lãi suất bình quân 6%/năm):
        $$\text{Dòng tiền Vùng 1} = 1.500.000.000 \times 6% = 90.000.000\text{ VNĐ/năm}$$
    *   Dòng tiền từ Vùng 2 (Tỷ suất cổ tức tiền mặt 6.5%/năm):
        $$\text{Dòng tiền Vùng 2} = 1.200.000.000 \times 6.5% = 78.000.000\text{ VNĐ/năm}$$
    *   Tổng dòng tiền mặt thực nhận hàng năm:
        $$\text{Tổng dòng tiền nhận được} = 90.000.000 + 78.000.000 = 168.000.000\text{ VNĐ/năm}$$

3.  **Bước 3: Đánh giá hiệu quả danh mục:**
    *   Tổng dòng tiền thực nhận hàng năm của chú Hùng đạt **168.000.000 VNĐ**, lớn hơn mốc mục tiêu chi tiêu **150.000.000 VNĐ** đề ra ban đầu (112%).
    *   Đồng thời, chú Hùng vẫn giữ nguyên được 100% quy mô vốn gốc 3 tỷ đồng, và có thêm khoản thặng dư tăng trưởng dài hạn của phần vốn 300 triệu đồng nằm ở Diamond ETF để chống lại sự bào mòn của lạm phát dài hạn.
    *   *Kết luận:* Danh mục thiết kế vô cùng hoàn hảo, bọc thép an toàn tuyệt đối và giải quyết trọn vẹn nỗi đau dòng tiền hưu trí cho chú Hùng.

---

## 📌 IV. BÀI TEST ĐÁNH GIÁ NĂNG LỰC (COMPETENCY ASSESSMENT)

*Hãy chọn đáp án đúng nhất cho các câu hỏi sau và đối chiếu với phần giải thích chi tiết ở dưới.*

### Câu 1: Nhu cầu tài chính cốt lõi hàng đầu của nhóm khách hàng High-NAV (tài sản lớn hơn 1 tỷ đồng) khi đầu tư chứng khoán là gì?
A. Muốn lướt sóng phái sinh đòn bẩy cao để làm giàu nhanh chóng.
B. Ưu tiên bảo tồn vốn an toàn trước lạm phát, chuyển giao tài sản bền vững và thiết lập danh mục phân bổ tài sản tạo dòng tiền đều đặn, thảnh thơi đầu óc.
C. Muốn được Broker bao che để giao dịch trái luật chứng khoán.
D. Chỉ muốn mua các cổ phiếu penny đang bị cảnh cáo giao dịch.

### Câu 2: Trong quy trình phỏng vấn CTV, tại sao kiến thức tài chính chứng khoán (Knowledge) chỉ chiếm trọng số 10% trong phiếu đánh giá ASK?
A. Vì dự án NextGen không coi trọng kiến thức chuyên môn.
B. Vì kiến thức lý thuyết có thể nhanh chóng đào tạo chuyển giao được qua tài liệu bọc thép của HO3 nếu ứng viên có thái độ chủ động học hỏi tốt và kỹ năng giao tiếp cơ bản.
C. Vì CTV không cần phải nói chuyện với khách hàng.
D. Vì kiến thức chứng khoán không ảnh hưởng đến hiệu quả kinh doanh.

### Câu 3: Ba phần cấu trúc bắt buộc phải có trong Báo cáo tốt nghiệp thực chiến NextGen HO3 là gì?
A. Giới thiệu bản thân – Ý kiến đóng góp về căn tin công ty – Đề xuất tăng lương.
B. Nhật ký thực chiến 42 ngày – Báo cáo hiệu quả định lượng phễu chuyển đổi (eKYC, active Standard, NAV, DTT) – Bài toán hoạch định tài sản Case Study thực tế.
C. Bản sao bằng tốt nghiệp đại học – Chứng chỉ phân tích kỹ thuật – Đơn xin việc.
D. Lịch sử giao dịch cá nhân – Bảng tính thuế phí – Lời cảm ơn.

### Câu 4: Khi chăm sóc khách hàng lớn High-NAV, hành động nào sau đây bị nghiêm cấm tuyệt đối theo quy tắc tuân thủ của KBSV?
A. Gửi báo cáo hiệu suất tài sản định kỳ hàng tháng cho khách hàng qua email.
B. Chụp ảnh màn hình số dư tài sản, nợ vay Margin hoặc lịch sử đặt lệnh của khách hàng gửi cho bên thứ ba hoặc đăng tải công khai lên mạng xã hội.
C. Tư vấn phân bổ danh mục Tháp tài sản 3 tầng của FinPeace.
D. Mời khách hàng tham gia các buổi Webinar vĩ mô chuyên sâu của HO3.

### Câu 5: Phong cách tư vấn "Private Banking Style" được đặc trưng bởi yếu tố nào sau đây?
A. Chăm sóc khách hàng đại trà bằng tin nhắn spam tự động hàng loạt.
B. Chăm sóc cá nhân hóa sâu sắc, điềm tĩnh lắng nghe mục tiêu cuộc đời của khách, thiết kế danh mục phân bổ tài sản Tailor-made riêng biệt và báo cáo định kỳ chỉn chu.
C. Cam kết chắc chắn bao lỗ 100% cho khách hàng lớn.
D. Chỉ phục vụ khách hàng vào ban đêm.

---

### 💡 ĐÁP ÁN & GIẢI THÍCH CHI TIẾT (ANSWER KEY)

*   **Câu 1: Chọn B.**
    *   *Giải thích:* Người có tài sản lớn họ sợ mất tiền hơn thèm kiếm tiền. Broker NextGen chinh phục họ bằng giải pháp bảo vệ tài sản bền vững, chứ không phải bằng các deal phím hàng rủi ro.
*   **Câu 2: Chọn B.**
    *   *Giải thích:* Thái độ quyết định hành vi. Một bạn CTV có thái độ tốt (Growth Mindset) sẽ học rất nhanh. Ngược lại, ứng viên giỏi kiến thức nhưng thái độ kiêu ngạo, lười biếng sẽ không thể làm việc nhóm và dễ gây rủi ro tuân thủ pháp lý.
*   **Câu 3: Chọn B.**
    *   *Giải thích:* Đây là cấu trúc chuẩn mực bọc thép của Báo cáo tốt nghiệp HO3 (nêu tại AGENTS.md), đảm bảo đánh giá toàn diện cả kỹ năng thực chiến và kết quả định lượng cụ thể của học viên NextGen.
*   **Câu 4: Chọn B.**
    *   *Giải thích:* Bảo mật thông tin khách hàng là nghĩa vụ pháp lý tối cao của người hành nghề chứng khoán được quy định rõ tại Luật Chứng khoán. Vi phạm quy định này sẽ lập tức bị sa thải và chịu trách nhiệm trước pháp luật.
*   **Câu 5: Chọn B.**
    *   *Giải thích:* Khách hàng lớn cần sự tinh tế, kín đáo và chuyên nghiệp cao nhất. Phong cách Private Banking Style nâng tầm Broker từ một nhân viên môi giới thông thường thành một Wealth Advisor đẳng cấp đồng hành cùng gia đình khách hàng.

---


---

## 📌 V. KỊCH BẢN ĐỐI LUYỆN (ROLE-PLAY & SCRIPT)

### 🎭 Tình huống: Gọi điện báo cáo định kỳ cho khách hàng VIP (NAV 5 tỷ) — Cá nhân hóa tư vấn Private Banking style
*   **Nhân vật:**
    *   **Broker NextGen HO3 (Học viên):** Chuyên nghiệp, chuẩn bị báo cáo cá nhân hóa, am hiểu nhu cầu High-NAV.
    *   **Cô Lan (Khách hàng VIP, NAV 5 tỷ):** 55 tuổi, chủ doanh nghiệp FDI. Cần báo cáo ngắn gọn, chính xác, không dài dòng. Quý trọng sự tôn trọng và riêng tư.

#### 📞 Đoạn Đối Thoại Mẫu (Script)

*   **Broker NextGen (giọng lịch sự, đúng giờ hẹn):** *Dạ cô Lan, em chào cô. Đúng 17h thứ Sáu như cô hẹn, em gọi báo cáo tuần cho cô ạ. Cô có 5 phút nghe em tóm tắt không ạ?*
*   **Cô Lan (giọng ngắn gọn, quyền lực):** *Nói ngắn gọn đi em. Tuần này danh mục cô thế nào?*
*   **Broker NextGen (report chuẩn Private Banking):** *Dạ vâng! Tổng NAV cô hiện tại: 5.28 tỷ, tăng 1.2% so với tuần trước. Cổ phiếu: FPT tăng 3%, ACB tăng 1.5%, VNM giảm nhẹ 0.8%. Cổ tức: ACB vừa chốt quyền chia cổ tức 15% bằng cổ phiếu, cô sẽ nhận thêm 1.200 cổ ACB vào tháng sau. Phần ETF Diamond chiếm 30% danh mục — ổn định, không cần điều chỉnh. Em có 1 khuyến nghị: Research KBSV vừa ra báo cáo nâng mục tiêu giá FPT lên 165k từ 145k. Em đề xuất cô giữ nguyên FPT, không chốt lời sớm ạ.*
*   **Cô Lan (hài lòng):** *Tốt. Còn gì nữa không?*
*   **Broker NextGen (thêm giá trị):** *Dạ, tuần tới có phiên đáo hạn phái sinh VN30F2506 vào thứ Năm — thường biến động mạnh. Em sẽ theo dõi sát và nhắn tin cô nếu cần điều chỉnh. Ngoài ra, KBSV vừa ra gói ưu đãi phí giao dịch 0.08% cho khách VIP — em đã đăng ký cho cô rồi, có hiệu lực từ thứ Hai tuần sau ạ.*
*   **Cô Lan (đánh giá cao):** *Em làm tốt lắm. Cứ giữ như thế nhé. Tuần sau báo cáo tiếp.*

#### 📊 Bảng Tiêu Chí Đánh Giá Năng Lực Sắm Vai (Grading Rubric dành cho Mentor)

| Tiêu Chí Đánh Giá | Điểm Tối Đa | Yêu Cầu Đạt Được (KPI) | Chấm Điểm Thực Tế |
| :--- | :---: | :--- | :---: |
| **Báo cáo cá nhân hóa (Report Quality)** | **40%** | NAV chính xác, % thay đổi, cổ tức, sự kiện sắp tới; ngắn gọn đúng 5 phút. | / 40 |
| **Phong cách Private Banking (Service Quality)** | **35%** | Gọi đúng giờ hẹn; xưng hô tôn trọng; proactive (đăng ký ưu đãi hộ KH). | / 35 |
| **Khuyến nghị dựa trên Research KBSV** | **25%** | Trích dẫn báo cáo Research; không tự ý khuyến nghị ngoài nguồn chính thức. | / 25 |

*Ngưỡng đạt yêu cầu: ≥ 80/100 điểm. Học viên dưới 80 điểm bắt buộc phải thực hành lại kịch bản đối luyện.*

## 📌 VI. QUY CHUẨN TUÂN THỦ PHÁT NGÔN (COMPLIANCE WARNINGS)

> [!IMPORTANT]
> **QUY TẮC BẢO MẬT THÔNG TIN KHÁCH HÀNG LỚN:**
> 1.  Học viên **tuyệt đối không** được trao đổi thông tin chi tiết về tài sản hoặc mã cổ phiếu nắm giữ của khách hàng High-NAV trong các nhóm chat Zalo/Messenger chung có sự tham gia của CTV sinh viên.
> 2.  Mọi tài liệu báo cáo danh mục gửi cho khách hàng lớn phải được mã hóa tên khách hàng (Ví dụ: Khách hàng Nguyễn Văn A ghi tắt là Khách hàng N.V.A) để ngăn ngừa rủi ro rò rỉ dữ liệu cá nhân trái phép.
